...
pub async fn ax_set_value(params: AxSetValueParams, session: Arc<AxSession>) -> CallToolResult {
    ...
    let ax_ref: AXRef = match session.lookup(&params.uid).await {
        ...
        Err(LookupError::SnapshotExpired { reason }) => {
            return ax_response::error("snapshot_expired", &reason, None);
        }
    ...
    // Capture pre-dispatch bbox so `not_dispatchable` / `ax_error` still get a
    // fallback centre if the element disappears mid-dispatch.
    let pre_bbox = unsafe { element_bbox(ax_ref.as_raw()) };

    match set_value_attribute(&ax_ref, &params.text) {
        Ok(()) => {
            // On success, return the element's CURRENT bbox for telemetry. The
            // snapshot's bbox remains the pre-action animation source on the
            // client side. Fall back to pre_bbox if the element no longer
            // exposes position+size post-dispatch.
